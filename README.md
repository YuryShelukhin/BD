...
1.3*. В OLAP базе документы хранить нельзя, так как она колонкоориентированная модель. SQL базы позволяют хранить документы и даже фотографии в бинарном коде, однако это требует дополнительного конфигурирования настроек для использования типа данных BLOB.  
...
2.1*. Для автоплатежа.
- настройка  задачи (включение автоплатежа, выборт получателя, сумма пополнения, счет списания, периодичность, где и что запрашивать и т.д.) в планировщике, обслуживающем счет, через платежное приложение или иным  способом, либо направление запроса в банке-эквайер на самостоятельное оповещение при снижении остатка; Детали получения инфомации пользователю не видны ("под капотом");
...
- проверка в платежной системе;
- проверка в банке-эквайере (куда поступят деньги), получение ответа;
- проверка информации в банке-эмитенте (откуда поступят деньги), получение ответа на возможность перевода (пароль, отсуствие блокировки, наличие денег);
- передача информации из банка-эмитента в платежную систему; 
- передача информации из платежной системы в банк-эквайер;
- увеличение суммы на счету в банке-эквайере;
- обратная передача информации в банк-эмитент об удачном завершении транзакции; 
